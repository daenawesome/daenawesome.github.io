                ...
                with open("life-expectancy.csv", "r") as f:
                    ...
                    while True:
                        ...
                        if selection == 1:
                            ...
                            # Compare the life expectancy of two or more countries over time
                            countries = input("Enter the countries you want to compare separated by comma (e.g. 'Afghanistan,Albania,Algeria'): ").split(',')
                            data = {}
                            with open("life-expectancy.csv", "r") as f:
                                reader = csv.DictReader(f)
                                for row in reader:
                                    if row['Entity'] in countries:
                                        if row['Entity'] not in data:
                                            data[row['Entity']] = {}
                                        data[row['Entity']][row['Year']] = row['Life expectancy (years)']
                            if not data:
                                print("No data found for the specified countries.")
                            else:
                                print("Life expectancy over time for the specified countries:")
                                for country in data:
                                    print(f"\n{country}: \n{data[country]}")
                        else:
