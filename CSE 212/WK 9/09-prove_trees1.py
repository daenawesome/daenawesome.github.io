...
class BST:
    ...
    def _insert(self, data, node):
        ...
        """
        # If the data is already present in the BST, 
    ...
    def _contains(self, data, node):
        ...
        """
        # If the node is None, the data is not found in the BST
        if node is None:
            return False  
        
        # If the data is found in the current node, return True
        if data == node.data:
            return True  
        ...
        if data < node.data:
            ...
            # Otherwise, recursively check in the right subtree
    ...
    def _traverse_backward(self, node):
        ...
        """
        if node is not None:
            yield from self._traverse_backward(node.right)  # Recursively traverse the right subtree
            yield node.data                                 # Yield the current node's data
            yield from self._traverse_backward(node.left)   # Recursively traverse the left subtree
...
def _insert_middle(sorted_list, first, last, bst):
    ...
    """
    # Base case: If the first index exceeds the last index, stop the recursion
    if first > last:
        return  
    ...
    # Recursively insert elements from the left half of the list
    _insert_middle(sorted_list, first, mid - 1, bst)  
    # Recursively insert elements from the right half of the list
    _insert_middle(sorted_list, mid + 1, last, bst)  
