...
class Taking_Turns_Queue:
    ...
    def get_next_person(self):
        ...
        if self.people.is_empty():
            ...
            # if person.turns > 1:
            if person.turns > 1 or person.turns <= 0:  
                person.turns -= 1 
                self.people.enqueue(person)
            print(person.name)
...
# Actual Before fix: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Tim, Sue
# Defect(s) Found: The order of the last two people in the queue was reversed. Sue should have 
#                   been dequeued first because she has more turns left.
...
while len(players) > 0:
    players.get_next_person()
# Actual Before fix: Bob, Tim, Sue, Bob, Tim, Sue, Tim, George, Sue, Tim, George, Tim, Sue, George
# Defect(s) Found: After adding George to the queue, the order was incorrect. Sue was dequeued 
#                   again before Tim had a chance to go again.
...
for i in range(10):
    players.get_next_person()
#print(players)    
# Actual Before fix: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Tim, Sue, Tim
# Defect(s) Found: The order of the last two people in the queue was reversed. Sue should have 
#                   been dequeued first because she has more turns left.
...
players = Taking_Turns_Queue()
players.get_next_person()
# Actual Before fix: No one in the queue.
# Defect(s) Found: The error message was not displayed correctly? N/A, Error Message Displayed
