...
// Image Upload PHP Motors Model
    ...
    function getImagePath($invId) {
        $db = phpmotorsConnect();
        $sql = 'SELECT imgPath, inventory.invId, invMake, invModel FROM images JOIN inventory ON images.invId = inventory.invId WHERE images.invId = :invId AND imgPath LIKE "%-tn%"';
        $stmt = $db->prepare($sql);
        $stmt->bindValue(':invId', $invId, PDO::PARAM_STR);
        $stmt->execute();
        $imageArray = $stmt->fetchAll(PDO::FETCH_ASSOC);
